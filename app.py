...
# -------------------------
# EXACT MODEL (FROM YOUR TRAINING)
# -------------------------
def DeeplabV3(input_shape=(256,256,3), num_classes=1):
    ...
    b4 = tf.keras.layers.Conv2D(256,1,padding='same')(b4)
    b4 = tf.keras.layers.UpSampling2D(size=(8,8), interpolation='bilinear')(b4)

    b0 = tf.keras.layers.Conv2D(256,1,padding='same')(layers[-1])
    b1 = tf.keras.layers.Conv2D(256,3,padding='same', dilation_rate=6)(layers[-1])
    b2 = tf.keras.layers.Conv2D(256,3,padding='same', dilation_rate=12)(layers[-1])
    b3 = tf.keras.layers.Conv2D(256,3,padding='same', dilation_rate=18)(layers[-1])
